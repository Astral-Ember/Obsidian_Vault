...
```
这里tp的类型为TableTennisPlayer&，意味着将调用基类的复制构造函数，但是基类没有定义复制构造函数，在需要复制构造函数但又没有定义的情况下，编译器将自动生成一个，此时会执行隐式复制构造函数
...
### 成员[[零散知识点#^bd45b7|初始化列表]]
派生类构造函数可以使用初始化器列表机制将值传递给基类构造函数，因为基类子对象的初始化在派生类构造函数体执行之前完成，只有在初始化列表中才能指定要用哪一个基类构造函数以及传入的参数

tips：如果想在派生类中直接使用基类中的构造函数，可以通过 using 关键字，显式的引入基类的构造函数
`using 父类名::父类构造函数名; `
它的作用相当于让子类中多了一个和父类一样的构造函数 

## 3.派生类与基类之间的特殊关系
派生类与基类之间有一些特殊关系。其中之一是派生类对象可以使用基类的方法，条件是：方法不是私有的
另外两个重要的关系是：
...
就是说：派生类继承自基类（继承了基类的方法），所以可以使用基类引用/指针为派生类对象调用基类的方法；但是如果反过来，派生类可以定义自己的方法，派生类引用/指针为基类对象调用派生类的方法，但是基类中并没有此类方法，此时会出现问题
